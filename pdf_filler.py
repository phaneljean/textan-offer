...
GENERATOR_VERSION = "2026-08-17.2"
...
def fill_offer_pdf(parsed: dict, agent_phone: str) -> str:
    ...
    # Footer on every TREC/financing-addendum page (not the cover page, which
    # already carries its own branded footer). TREC's own "Initialed for
    # identification..." line sits ~32pt above the bottom edge on every page
    # (confirmed by extracting each page's lowest text run) -- this footer
    # is kept below that, in the clear margin under it.
    from reportlab.lib.pagesizes import letter
    ...
    for i in range(1, total_pages):
        ...
        overlay_buf = io.BytesIO()
        c = rl_canvas.Canvas(overlay_buf, pagesize=letter)
        c.setStrokeColorRGB(0.102, 0.122, 0.180)  # #1A1F2E
        c.setLineWidth(0.5)
        c.line(0.65 * inch, 0.30 * inch, letter[0] - 0.65 * inch, 0.30 * inch)
        c.setFillColorRGB(0.420, 0.447, 0.502)  # #6B7280
        c.setFont("Helvetica", 7)
        c.drawCentredString(
            letter[0] / 2, 0.14 * inch,
            f"Generated by TxtAnOffer  ·  txtanoffer.com  ·  Page {page_num} of {total_pages}  ·  "
            f"Not affiliated with TREC. Agent must review all fields.  ·  gen v{GENERATOR_VERSION}"
        )
        c.save()
        overlay_buf.seek(0)
        final_writer.pages[i].merge_page(PdfReader(overlay_buf).pages[0])

    # Also embed the version in document metadata (queryable even if the
    # visible footer text is ever redesigned or a page gets cropped/printed).
